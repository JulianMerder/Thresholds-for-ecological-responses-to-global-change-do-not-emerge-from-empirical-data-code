meta_marginal <- function(data,tbw=0.33,np="original"){
  source("my_kde1d.R")
  ...
  if (data$marg$ysum < 0.99) data$facy=data$facy+0.3
  if (data$marg$ysum < 0.99) cat(paste0("increased y range factor in run ",np," to ",data$facy,"!","\n"))
  if (data$marg$ysum >= 0.99) data$checkfacy="passed"
  #if (data$marg$ysum < 0.90) stop("increase facy!")
  #plot(data$grid$gy,data$marg$y)
  return(data)
}